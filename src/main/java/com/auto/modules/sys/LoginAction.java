...
import com.auto.common.util.OnlineUtil;
import com.auto.modules.sys.bean.Sys_safeconfig;
import com.auto.modules.sys.bean.Sys_unit;
...
//			return "验证码不正确！";
    ...
    @AdaptBy(type=WhaleAdaptor.class)
    @At("/userLogin")
    @Ok("vm:template.html.userProfile")
    public String userLogin(@Param("loginname") String loginname,
                        @Param("password") String password,
                        @Param("verifcode") String verifcode, HttpSession session,
                        HttpServletRequest req) {
        if (Strings.isBlank(loginname) || Strings.isBlank(password))
